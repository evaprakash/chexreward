...
class ImageReward(nn.Module):
    ...
    def score_gard(self, image, text):
        
        text_input = self.blip.tokenizer(text, padding='max_length', truncation=True, max_length=35, return_tensors="pt").to(self.device)

        image_embeds = self.blip.visual_encoder(image)
        image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(self.device)

        text_output = self.blip.text_encoder(text_input.input_ids, attention_mask = text_input.attention_mask, encoder_hidden_states = image_embeds, encoder_attention_mask = image_atts, return_dict = True)
    ...
    def forward(self, batch_data):
        ...
        # encode data
        if opts.rank_pair:
            text_cond, mask_cond, rank_t_1, rank_t_2 = batch_data['orig_text'], batch_data['orig_conditioning_image'], batch_data['rank_t_1'], batch_data['rank_t_2']
        ...
        reward_1 = self.mlp(emb_1)
        reward_2 = self.mlp(emb_2)

        reward = torch.concat((reward_1, reward_2), dim=1)
        if rank_t_1 < rank_t_2:
            reward = torch.concat((reward_2, reward_1), dim=1)
        
        return reward

    def encode_pair(self, batch_data):
        orig_text, orig_conditioning_image, conditioning_image, text_ids, text_mask, img_1, img_2, rank_t_1, rank_t_2, rank_m_1, rank_m_2 = batch_data['orig_text'], batch_data['orig_conditioning_image'], batch_data['conditioning_image'], batch_data['text_ids'], batch_data['text_mask'], batch_data['image_1'], batch_data['image_2'], batch_data['rank_t_1'], batch_data['rank_t_2'], batch_data['rank_m_1'], batch_data['rank_m_2']
        ...
        with open("temp.txt", "w") as fp:
            fp.write(str(self.score_gard(img_1, orig_text)))
